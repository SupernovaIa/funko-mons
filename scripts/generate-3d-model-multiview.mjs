#!/usr/bin/env node
// Usage: node --env-file=.env.local scripts/generate-3d-model-multiview.mjs <slug> <front> <back> [left] [right]
...
if (!API_KEY) {
  console.error("Missing TRIPO_API_KEY. Run with: node --env-file=.env.local <script> ...");
  process.exit(1);
}

const [slug, frontPath, backPath, leftPath, rightPath] = process.argv.slice(2);
if (!slug || !frontPath) {
  console.error("Usage: generate-3d-model-multiview.mjs <slug> <front> [back] [left] [right]");
  process.exit(1);
}
...
async function createTask(files) {
  const res = await fetch(`${BASE_URL}/generation/multiview-to-model`, {
    method: "POST",
...
// Order required by the API: [front, left, back, right]
const paths = [frontPath, leftPath, backPath, rightPath];
const files = [];
for (const p of paths) {
  ...
  const token = await uploadImage(p);
  console.log(`Uploaded ${p} -> ${token}`);
